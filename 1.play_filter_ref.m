...
% Step 1: load file, filter, referencing, run ica, apply channel location
    ...
    eeglab('redraw');
    EEG = pop_loadset([rawdir subject filesep datasetname '.set']);
    %EEG = pop_loadset([rawdir subject filesep datasetname '.set'], 'streamtype', 'EEG', 'exclude_markerstreams', {});
...
%     
%     [ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
%     EEG = pop_loadset([workdir subject filesep datasetname '_fl_rr.set'])
%     %EEG = pop_loadset('filename',[subject CURRENTSET '_fl_rr'],'filepath',[workdir subject filesep]);
%     [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
%     EEG = pop_runica(EEG, 'extended',1,'interupt','on');
%     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET,'setname',[datasetname '_fl_rr_ica'],'gui','off');
%     EEG = pop_saveset( EEG, [workdir subject filesep datasetname '_fl_rr_ica']);
% end
...
%     EEG = eeg_checkset( EEG );
%     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET,'gui','off'); 
%     EEG  = pop_binlister( EEG , 'BDF', [txtdir 'play_binlist.txt'], 'ExportEL', [txtdir subject filesep 'binlist.txt'],'ImportEL', [txtdir subject filesep 'eventlist.txt'], 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'Voutput', 'EEG' );
